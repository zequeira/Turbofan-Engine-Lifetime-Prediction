...
# Training Function
def train_model_v2(model, loss_function, optimizer, scheduler, num_epochs=25):
    since = time.time()
    best_loss = 1e10
    best_model_wts = copy.deepcopy(model.state_dict())
    loss_history = {'train': [], 'val': []}
...
class LSTM_RUL_Estimator(nn.Module):
    ...
    def __init__(self, n_features, hidden_dim=50, dropout=0.2, seq_length=40, num_layers=2, output_dim=1):
...
class CMAPSSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # data = self.df_data[idx:idx + self.seq_len]
...
if __name__ == '__main__':
    ...
    best_model = LSTM_RUL_Estimator(n_features=sample_data.shape[2],
                                    hidden_dim=100, dropout=0.5,
                                    seq_length=sequence_length,
                                    num_layers=3, output_dim=1)
